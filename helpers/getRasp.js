...
const getRasp = async (ctx, setDay) => {
  ...
  if ((dateHelper.today === 5 || dateHelper.today === 6) && setDay === 9)
    return await ctx.reply("А завтра, вроде бы как, выходной 🙃"); // Если пятница и мы жмем на кнопку "Завтра"
  if ((dateHelper.today === 6 || dateHelper.today === 0) && setDay === 8) {
    return await ctx.reply("Как бы выходной, не? 🙃"); // Если суббота или воскр. и жмем на кнопку "Сегодня"
  }
