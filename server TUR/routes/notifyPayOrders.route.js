  ...
  if (status === "ERROR") {
    try {
      await bot.telegram.sendMessage(
        comment,
        `Ödeme №${txnId} tamamlanmadı. İşlem durumunu netleştirmek için lütfen destek ile iletişime geçin.
Destek: @LuckyCatGames`
      );
      await Users.updateOne({ comment }, { $inc: { mainBalance: amount } });
      await bot.telegram.sendMessage(
        comment,
        `Kesilen tutarın iadesi: ${amount} TL`
      );
    } catch (error) {
  ...
  if (status === "WAITING") {
    try {
      await bot.telegram.sendMessage(
        comment,
        `№${TxnId} numaralı ödemeniz işlenmek üzere kabul edildi. Lütfen bekle.`
      );
    } catch (error) {
...
async function inCash(txnId, amount, userId) {
  ...
  if (user.isRef !== 0) {
    ...
    try {
      await bot.telegram.sendMessage(
        user.isRef,
        `Davet ettiğiniz kişi için ana hesabınıza ${(
          (amount * bonusRefPercent) /
          100
        ).toFixed(2)} TL ödeme yapıldı.
Ödeme numarası: ${txnId}`
      );
    } catch (error) {}
  }
  ...
  try {
    await bot.telegram.sendMessage(
      userId,
      `Bakiyeniz başarıyla aktarıldı: ${amount}₽.
Mevcut bakiyeniz: ${user.mainBalance + amount}
...
Mevcut bakiyeniz: ${user.mainBalance}.

Mevcut bakiyeniz: ${txnId}`
  );
  ...
  await nodeHtmlToImage({
    ...
    <body>
      <div class="card">
        <div class="title">Operasyon başarılı oldu</div>
        <div class="date">
          <div class="name">Tarih</div>
          <div class="time">${moment().format("HH:mm DD.MM.YYYY")}</div>
        </div>
        <div class="number-order">
          <div class="name">İşlem numarası:</div>
          <div class="no">${txnId}</div>
        </div>
        <div class="number-order">
