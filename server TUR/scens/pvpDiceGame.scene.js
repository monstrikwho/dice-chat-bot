...
pvpDiceGame.action("Lobilerim", async (ctx) => {
...
pvpDiceGame.action("Lobi oluştur", async (ctx) => {
  const { actionStatus } = ctx.session.state;
...
pvpDiceGame.action(/(?:2 oyuncu|3 oyuncu|5 oyuncu)/, async (ctx) => {
  const { actionStatus } = ctx.session.state;
...
pvpDiceGame.action("Lobiye katıl", async (ctx) => {
  const { actionStatus } = ctx.session.state;
...
pvpDiceGame.action("Lobiden çık", async (ctx) => {
  const { actionStatus } = ctx.session.state;
...
pvpDiceGame.action("🔄 Güncelle", async (ctx) => {
...
pvpDiceGame.action("Geri", async (ctx) => {
    ...
    Extra.markup((m) =>
      m.inlineKeyboard([
        [
          ...
          m.callbackButton("3 oyuncu", "3 oyuncu"),
          m.callbackButton("5 oyuncu", "5 oyuncu"),
        ],
        [m.callbackButton("Geri", "Geri")],
      ])
    )
